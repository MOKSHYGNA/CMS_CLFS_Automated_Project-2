...
def create_dme_pen_state_table(conn):
...
def get_dme_pen_files():
    ...
    for file_path in DME_ROOT.rglob("DMEPEN*.csv"):
...
# ============================================================
# LOAD DME PEN STATE PRICING
# ============================================================

def load_dme_pen_state_pricing():

    print()
    print("=" * 60)
    print("DME PEN STATE PRICING LOADER")
    print("=" * 60)

    csv_files = get_dme_pen_files()

    print(
        f"Found DMEPEN CSV files: {len(csv_files)}"
    )

    if not csv_files:

        print(
            "No DMEPEN CSV files found."
        )
    ...
    create_dme_pen_state_table(conn)

    cursor = conn.cursor()

    # Clear old data so the loader is rerunnable.
    cursor.execute(
        "DELETE FROM dme_pen_state_pricing"
    )
    ...
    for file_path in csv_files:
        ...
        try:
            ...
            cursor.executemany(
                """
                INSERT INTO dme_pen_state_pricing (
                    hcpcs,
                    mod,
                    mod2,
                    state,
    ...
    print()
    print("=" * 60)
    print("DME PEN STATE PRICING SUMMARY")
    print("=" * 60)
